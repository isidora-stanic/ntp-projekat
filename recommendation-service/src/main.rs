...
fn new_param(req: Json<RecommendationParamDTO>) -> Json<String> {
    thread::spawn(move || {
        let params = service::create(req.into_inner());
        Json(params.unwrap())
    }).join().unwrap()
}
...
fn update_param(id: i32, req: Json<RecommendationParamDTO>) -> Json<String> {
    thread::spawn(move || {
        let params = service::update(id, req.into_inner());
        Json(params.unwrap())
    }).join().unwrap()
}
...
async fn get_recommendations(product: Json<Product>) -> Json<Vec<RecommendedProducts>> {
    ...
    let params_finish = thread::spawn(move || {
    ...
    let params_color = thread::spawn(move || {
    ...
    let params_material = thread::spawn(move || {
